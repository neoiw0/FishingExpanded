...
namespace FishingExpanded.Patches
{
    ...
    internal class ObjectPatches
    {
        ...
        /// <summary>记录原生虚方法是否实际进入手持物品绘制。</summary>
        [HarmonyPatch(nameof(StardewValley.Object.drawWhenHeld))]
        [HarmonyPrefix]
        public static void DrawWhenHeld_Prefix(
            StardewValley.Object __instance,
            Farmer f,
            ref Vector2 objectPosition)
        {
            try
            {
                ...
                float visualScale = GiantFishManager.GetFishVisualScale(fishId, f);
                if (visualScale <= 1.001f)
                    return;

                // Native drawWhenHeld uses Vector2.Zero as the origin. Keep the
                // original fish bottom-center point fixed while enlarging it.
                var itemData = ItemRegistry.GetDataOrErrorItem(fishId);
                Rectangle sourceRect = itemData.GetSourceRect(0, __instance.ParentSheetIndex);
        ...
        public static float GetDrawScale(StardewValley.Object obj, Farmer owner)
        {
            try
            {
                string fishId = obj?.QualifiedItemId ?? "<null>";
                int? category = obj?.Category;
                bool isFishCategory = category == StardewValley.Object.FishCategory;
                float visualScale = 1.0f;

                // 保持生产过滤逻辑不变；本轮先区分“未进入鱼类分支”和“状态未命中”。
