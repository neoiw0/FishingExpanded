...
namespace FishingExpanded
{
	public class ModEntry : Mod
	{
		...
		private void OnCommandSetLevel(string command, string[] args)
		{
			...
			DifficultyManager.SetDifficultyLevel(text, result);
			int difficultyLevel = DifficultyManager.GetDifficultyLevel(text);
		...
		private void OnCommandAddSuccess(string command, string[] args)
		{
			...
			{
				DifficultyManager.RecordSuccess(text);
			}
			int difficultyLevel = DifficultyManager.GetDifficultyLevel(text);
		...
		private void OnCommandAddFail(string command, string[] args)
		{
			...
			for (int i = 0; i < result; i++)
			{
				DifficultyManager.RecordFailure(text);
			}
			int difficultyLevel = DifficultyManager.GetDifficultyLevel(text);
		...
		private void OnCommandInfo(string command, string[] args)
		{
			...
			string text = args[0];
			FishStats fishStats = DifficultyManager.GetFishStats(text);
			if (fishStats == null)
			{
			...
			bool flag = DifficultyManager.HasCollectionStar(text);
		...
		{
			Dictionary<string, FishStats> allFishStats = DifficultyManager.GetAllFishStats();
			...
			foreach (KeyValuePair<string, FishStats> item in allFishStats)
			{
				...
				string value2 = (DifficultyManager.HasCollectionStar(key) ? " ★" : "");
		...
		private void OnCommandAddStar(string command, string[] args)
		{
			...
			string text = args[0];
			DifficultyManager.RecordHighDifficulty(text);
		...
		{
			float fishingLevelBonus = DifficultyManager.GetFishingLevelBonus();
			int value = (int)Math.Floor(fishingLevelBonus);
			int collectionStarCount = DifficultyManager.GetCollectionStarCount();
			((Mod)this).Monitor.Log("===========================================", (LogLevel)2);
...
namespace FishingExpanded.Services
{
	public static class DifficultyManager
	{
		...
		public static void LoadData()
		{
			...
			if (Game1.player != null && ((NetDictionary<string, string, NetString, SerializableDictionary<string, string>, NetStringDictionary<string, NetString>>)(object)((Character)Game1.player).modData).TryGetValue("FishingExpanded/FishDifficultyData", ref text) && !string.IsNullOrWhiteSpace(text))
			{
				try
				{
					_data = JsonSerializer.Deserialize<FishDifficultyData>(text, JsonOptions);
				}
				catch (JsonException ex)
				{
					ModEntry.ModMonitor.Log($"[DifficultyManager] 玩家存档数据解析失败，将使用空数据 | 玩家: {Game1.player.UniqueMultiplayerID} | 错误: {ex.Message}", (LogLevel)4);
				}
			}
			if (_data == null && Context.IsMainPlayer)
			{
				...
				{
					ModEntry.ModMonitor.Log("[DifficultyManager] 读取旧存档级数据失败，将使用空数据 | 错误: " + ex2.Message, (LogLevel)4);
				}
			}
			if (_data == null)
			{
			...
			defaultInterpolatedStringHandler.AppendLiteral("[DifficultyManager] 加载玩家数据完成 | 玩家: ");
			Farmer player = Game1.player;
			defaultInterpolatedStringHandler.AppendFormatted((player != null) ? new long?(player.UniqueMultiplayerID) : ((long?)null));
			defaultInterpolatedStringHandler.AppendLiteral(" | 鱼种类数: ");
			defaultInterpolatedStringHandler.AppendFormatted(_data.FishStatistics.Count);
			defaultInterpolatedStringHandler.AppendLiteral(" | ");
			defaultInterpolatedStringHandler.AppendLiteral("钓鱼等级加成: +");
			defaultInterpolatedStringHandler.AppendFormatted(_data.FishingLevelBonus, "F1");
			defaultInterpolatedStringHandler.AppendLiteral(" | 星标鱼种: ");
			defaultInterpolatedStringHandler.AppendFormatted(_data.CollectionStars.Count);
		...
		public static void SaveData()
		{
			...
			{
				NormalizeData();
				((NetDictionary<string, string, NetString, SerializableDictionary<string, string>, NetStringDictionary<string, NetString>>)(object)((Character)Game1.player).modData)["FishingExpanded/FishDifficultyData"] = JsonSerializer.Serialize(_data, JsonOptions);
				ModEntry.ModMonitor.Log($"已保存玩家钓鱼难度数据 | 玩家: {Game1.player.UniqueMultiplayerID}", (LogLevel)1);
			}
		}
		...
		{
			string text = SpecialFishHelper.NormalizeItemId(fishId);
			if (_data == null || string.IsNullOrEmpty(text) || SpecialFishHelper.IsLegendaryFish(text) || !_data.FishStatistics.TryGetValue(text, out var value))
			{
				return 0;
			}
		...
		public static int RecordSuccess(string fishId, int levelGain = 1)
		{
			if (_data == null)
			{
				ModEntry.ModMonitor.Log("[DifficultyManager] ERROR: _data为null，无法记录成功", (LogLevel)4);
				return 0;
			}
			string text = SpecialFishHelper.NormalizeItemId(fishId);
			...
			if (!_data.FishStatistics.TryGetValue(text, out var value))
			{
				value = new FishStats();
				_data.FishStatistics[text] = value;
				ModEntry.ModMonitor.Log("[DifficultyManager] 新鱼种记录创建: " + text, (LogLevel)1);
			}
			int difficultyLevel = GetDifficultyLevel(text);
			int maxDifficultyLevel = GetMaxDifficultyLevel(text);
			...
			int difficultyLevel2 = GetDifficultyLevel(text);
			ModEntry.ModMonitor.Log($"[DifficultyManager] 钓鱼成功记录 | 鱼ID: {text} | 请求增长: +{levelGain} | 实际增长: +{num2} | 成功次数: {value.SuccessCount} | 失败次数: {value.FailCount} | 等级变化: {difficultyLevel} → {difficultyLevel2}", (LogLevel)2);
			SaveData();
			return num2;
		}
		...
		public static void RecordFailure(string fishId)
		{
			if (_data == null)
			{
				ModEntry.ModMonitor.Log("[DifficultyManager] ERROR: _data为null，无法记录失败", (LogLevel)4);
				return;
			}
			string text = SpecialFishHelper.NormalizeItemId(fishId);
			if (!string.IsNullOrEmpty(text) && !SpecialFishHelper.IsLegendaryFish(text))
			{
				...
				int difficultyLevel2 = GetDifficultyLevel(text);
				ModEntry.ModMonitor.Log($"[DifficultyManager] 钓鱼失败记录 | 鱼ID: {text} | 成功次数: {value.SuccessCount} | 失败次数: {value.FailCount} | 等级变化: {difficultyLevel} → {difficultyLevel2}", (LogLevel)2);
				SaveData();
			}
		}
		...
		public static void RecordHighDifficulty(string fishId, float adjustedDifficulty)
		{
			if (_data == null)
			{
				ModEntry.ModMonitor.Log("[DifficultyManager] ERROR: _data为null，无法记录高难度", (LogLevel)4);
				return;
			}
			string text = SpecialFishHelper.NormalizeItemId(fishId);
			if (!string.IsNullOrEmpty(text) && !SpecialFishHelper.IsLegendaryFish(text) && adjustedDifficulty >= 120f && !_data.CollectionStars.Contains(text))
			{
				_data.CollectionStars.Add(text);
				_data.FishingLevelBonus = (float)_data.CollectionStars.Count * 0.5f;
				ModEntry.ModMonitor.Log($"[DifficultyManager] ★ 达成高难度里程碑 ★ | 鱼ID: {text} | 调整后难度: {adjustedDifficulty:F1} | 新增钓鱼等级加成: +0.5 | 总加成: +{_data.FishingLevelBonus:F1}", (LogLevel)3);
				SaveData();
			}
		}
		...
		{
			string text = SpecialFishHelper.NormalizeItemId(fishId);
			if (_data != null && !string.IsNullOrEmpty(text))
			{
				return _data.CollectionStars.Contains(text);
			}
			return false;
		}
		...
		public static void SetDifficultyLevel(string fishId, int targetLevel)
		{
			if (_data == null)
			{
				ModEntry.ModMonitor.Log("[DifficultyManager] ERROR: _data为null", (LogLevel)4);
				return;
			}
			string text = SpecialFishHelper.NormalizeItemId(fishId);
			if (!string.IsNullOrEmpty(text) && !SpecialFishHelper.IsLegendaryFish(text))
			{
				...
				ModEntry.ModMonitor.Log($"[DifficultyManager] 已设置等级 | 鱼ID: {text} | 目标等级: {targetLevel} | 成功次数: {value.SuccessCount} | 失败次数: {value.FailCount}", (LogLevel)2);
				SaveData();
			}
		}
		...
		public static Dictionary<string, FishStats> GetAllFishStats()
		{
			return _data?.FishStatistics ?? new Dictionary<string, FishStats>();
		}
		...
		public static void RecordHighDifficulty(string fishId)
		{
			...
			if (!string.IsNullOrEmpty(text) && !SpecialFishHelper.IsLegendaryFish(text))
			{
				if (!_data.CollectionStars.Contains(text))
				{
					_data.CollectionStars.Add(text);
					_data.FishingLevelBonus = (float)_data.CollectionStars.Count * 0.5f;
					ModEntry.ModMonitor.Log($"[DifficultyManager] ★ 添加收藏星标 ★ | 鱼ID: {text} | 新增钓鱼等级加成: +0.5 | 总加成: +{_data.FishingLevelBonus:F1}", (LogLevel)3);
					SaveData();
				}
				else
				{
		...
		private static void NormalizeData()
		{
			...
			foreach (KeyValuePair<string, FishStats> item in _data.FishStatistics ?? new Dictionary<string, FishStats>())
			{
			...
			foreach (string item2 in _data.CollectionStars ?? new HashSet<string>())
			{
			...
			_data.FishStatistics = dictionary;
			_data.CollectionStars = hashSet;
			_data.FishingLevelBonus = (float)hashSet.Count * 0.5f;
		}
	...
	public static class GiantFishManager
	{
		...
		public static void RecordGiantFish(Farmer player, string fishId, int multiplier, int fishSize)
		{
			...
			if (displayData != null && multiplier > 15 && !SpecialFishHelper.IsLegendaryFish(text) && IsFish(text))
			{
				...
				ModEntry.ModMonitor.Log($"[GiantFishManager] 超大鱼记录 | 玩家: {player.UniqueMultiplayerID} | 鱼ID: {text} | 倍数: {multiplier} | fishSize: {fishSize} | 视觉缩放: ×{Math.Pow(multiplier, 1.0 / 3.0):F2}", (LogLevel)2);
			}
		}
		...
		public static void CheckAndTriggerNPCReactions()
		{
			...
			if (activeObject == null || !Game1.player.IsCarrying())
			{
				return;
			}
			...
			List<NPC> nearbyNPCs = GetNearbyNPCs(((Character)Game1.player).Position, 320f);
			if (nearbyNPCs.Count > 0)
			{
				(long, string) key = (Game1.player.UniqueMultiplayerID, text);
				if (!_lastLoggedNearbyCheck.ContainsKey(key) || _lastLoggedNearbyCheck[key] != nearbyNPCs.Count)
				{
					ModEntry.ModMonitor.Log($"[GiantFishManager] 检测到附近NPC | 玩家: {Game1.player.UniqueMultiplayerID} | 鱼ID: {text} | 5格内NPC数量: {nearbyNPCs.Count}", (LogLevel)1);
					_lastLoggedNearbyCheck[key] = nearbyNPCs.Count;
				}
			}
			foreach (NPC item3 in nearbyNPCs)
			{
				TriggerNPCBubble(item3, text, item2);
			}
		}
	...
	public static class HUDNotifier
	{
		...
		public static void ShowStarChallengeNotification(string fishId, int difficultyLevel)
		{
			...
			if (!SpecialFishHelper.IsLegendaryFish(fishId) && DifficultyManager.HasCollectionStar(fishId))
			{
...
namespace FishingExpanded.Patches
{
	...
	internal class BobberBarPatches
	{
		...
		{
			FarmerFishingLevelPatches.SuppressHiddenBonusForLegendaryBobber = SpecialFishHelper.IsLegendaryFish(whichFish);
		}
		...
		public static void Constructor_Postfix(BobberBar __instance, string whichFish, ref float ___difficulty, ref int ___fishSize, ref int ___fishQuality, ref float ___distanceFromCatchPenaltyModifier, bool ___bobberInBar)
		{
			try
			{
				...
				HUDNotifier.ShowStarChallengeNotification(text, difficultyLevel);
		...
		public static float GetFishVisualScale(BobberBar instance)
		{
			if (!_instanceData.TryGetValue(instance, out var value))
			{
				return 1f;
			}
			float visualScale = DifficultyCalculator.GetVisualScale(value.QuantityMultiplier);
			ObjectPatches.LogVisualDiagnostic("BobberBar.draw", $"{value.PlayerId}:{value.FishId}", $"anchor=center | visualScale={visualScale:F3} | stateHit={visualScale > 1.001f}");
			return visualScale;
		}
		...
		public static IEnumerable<CodeInstruction> Draw_Transpiler(IEnumerable<CodeInstruction> instructions)
		{
			...
			MethodInfo methodInfo = AccessTools.Method(typeof(BobberBarPatches), "GetFishVisualScale", (Type[])null, (Type[])null);
			List<CodeInstruction> list2 = new List<CodeInstruction>(list.Count + 3);
			bool flag = false;
			int num = 0;
			foreach (CodeInstruction item in list)
			{
				if (item.opcode == OpCodes.Ldc_I4 && item.operand is int num2 && num2 == 1840)
				{
					flag = true;
				}
				list2.Add(item);
				if (flag && item.opcode == OpCodes.Ldc_R4 && item.operand is float num3 && Math.Abs(num3 - 2f) < 0.001f)
				{
			...
			ModEntry.ModMonitor.Log((num == 1) ? "[BobberBarPatches] 已接入小游戏鱼标视觉缩放；原点保持中心" : $"[BobberBarPatches] 小游戏鱼标视觉缩放注入数: {num}", (LogLevel)((num == 1) ? 2 : 3));
			return list2;
		}
			...
			try
			{
				if (_instanceData.TryGetValue(__instance, out var value))
				{
					if (___fadeOut && ___distanceFromCatching <= 0f && !value.FailureRecorded)
					{
						...
						DifficultyManager.RecordFailure(value.FishId);
						int difficultyLevel = DifficultyManager.GetDifficultyLevel(value.FishId);
		...
		public static void CreateDescription_Postfix(CollectionsPage __instance, string id, ref string __result, int ___currentTab)
		{
			try
			{
				if (___currentTab == 1 && !string.IsNullOrEmpty(id) && !SpecialFishHelper.IsLegendaryFish(id))
				{
					...
					{
						list.Add($"钓鱼技能加成：+{fishingLevelBonus:F1}");
					}
					if (list.Count > 0)
					{
		...
		public static void Draw_Postfix(SpriteBatch b, int ___currentTab, int ___currentPage, Dictionary<int, List<List<ClickableTextureComponent>>> ___collections)
		{
			...
			try
			{
				...
				foreach (ClickableTextureComponent item in value[___currentPage])
				{
					...
					if (array.Length != 0 && DifficultyManager.HasCollectionStar(array[0]))
					{
	...
	internal class FarmerFishingLevelPatches
	{
		...
		public static void FishingLevel_Getter_Postfix(Farmer __instance, ref int __result)
		{
			try
			{
				if (__instance != Game1.player || SuppressHiddenBonusForLegendaryBobber)
				{
					return;
				}
				float fishingLevelBonus = DifficultyManager.GetFishingLevelBonus();
				if (fishingLevelBonus > 0f)
				{
					...
					__result += num;
					if (Math.Abs(fishingLevelBonus - _lastLoggedBonus) > 0.01f)
					{
						_lastLoggedBonus = fishingLevelBonus;
			...
			{
				ModEntry.ModMonitor.Log($"FishingLevel Getter Patch 失败: {value2}", (LogLevel)4);
			}
		}
	...
	internal class FishingRodPatches
	{
		...
		public static Vector2 AdjustFirstSettlementFishPosition(Vector2 position, FishingRod rod)
		{
			...
			try
			{
				Rectangle sourceRect = rod.whichFish.GetParsedOrErrorData().GetSourceRect(0, (int?)null);
				return position + new Vector2((float)sourceRect.Width * 4f * (1f - fishVisualScale) / 2f, (float)sourceRect.Height * 4f * (1f - fishVisualScale) / 2f);
			}
			catch (Exception value)
			{
				ModEntry.ModMonitor.Log($"[FishingRodPatches] 结算中心锚点补偿失败: {value}", (LogLevel)3);
				return position;
			}
		}
		...
		public static IEnumerable<CodeInstruction> Draw_Transpiler(IEnumerable<CodeInstruction> instructions)
		{
			...
			MethodInfo methodInfo2 = AccessTools.Method(typeof(FishingRodPatches), "AdjustFirstSettlementFishPosition", (Type[])null, (Type[])null);
			int num = list.FindIndex((CodeInstruction code) => IsLdcI4(code, 1870));
			if (num < 0)
			{
				ModEntry.ModMonitor.Log("[FishingRodPatches] 未找到结算第一处鱼图源矩形，视觉锚点未注入", (LogLevel)3);
				return list;
			}
			int num2 = -1;
			for (int num3 = 0; num3 < num; num3++)
			{
				if (IsGame1GlobalToLocalCall(list[num3]))
				{
					num2 = num3;
				}
			}
			int num4 = -1;
			int num5 = -1;
			for (int num6 = num; num6 < list.Count; num6++)
			{
				if (num4 < 0 && IsLdcR4(list[num6], 4f))
				{
					num4 = num6;
				}
				if (IsSpriteBatchDrawCall(list[num6]))
				{
					num5 = num6;
					break;
				}
			...
			if (num5 >= 0)
			{
				for (int num8 = num; num8 < num5; num8++)
				{
					if (IsNullableRectangleConstructor(list[num8]) && num8 != 0)
					{
						num7 = GetLocalIndex(list[num8 - 1]);
						if (num7 >= 0)
						{
							break;
						}
			...
			{
				num7 = 28;
				ModEntry.ModMonitor.Log("[FishingRodPatches] 未能从 Nullable<Rectangle> 自动读取鱼图局部变量，使用当前安装契约 local 28", (LogLevel)1);
			}
			List<CodeInstruction> list2 = new List<CodeInstruction>(list.Count + 16);
			bool flag = false;
			int num9 = 0;
			for (int num10 = 0; num10 < list.Count; num10++)
			{
				CodeInstruction val = list[num10];
				list2.Add(val);
				if (num10 == num2)
				{
					list2.Add(new CodeInstruction(OpCodes.Ldarg_0, (object)null));
				...
				if (num10 > num5 && num7 >= 0 && IsLoadLocal(list[num10], num7) && num10 + 1 < list.Count && IsNullableRectangleConstructor(list[num10 + 1]))
				{
					flag = true;
				}
				if (num10 > num5 && flag && IsLdcR4(val, 3f))
				{
					list2.Add(new CodeInstruction(OpCodes.Ldarg_0, (object)null));
					list2.Add(new CodeInstruction(OpCodes.Call, (object)methodInfo));
					list2.Add(new CodeInstruction(OpCodes.Mul, (object)null));
			...
			int num11 = ((num2 >= 0) ? 1 : 0);
			int num12 = ((num4 >= 0) ? 1 : 0);
			ModEntry.ModMonitor.Log($"[FishingRodPatches] 结算视觉锚点注入 | 中心位置补偿: {num11} | 首图缩放: {num12} | 中心鱼图缩放: {num9}", (LogLevel)((num11 == 1 && num12 == 1 && num9 >= 1) ? 2 : 3));
			return list2;
		}
		...
		private static bool IsNullableRectangleConstructor(CodeInstruction code)
		{
			if (code.opcode != OpCodes.Newobj || !(code.operand is ConstructorInfo constructorInfo))
			{
				return false;
			}
		...
		{
			return GetLocalIndex(code) == expectedIndex;
		}
		...
		private static int GetLocalIndex(CodeInstruction code)
		{
			...
			return GetLocalIndex(code.operand);
		}

		private static int GetLocalIndex(object operand)
		{
			if (!(operand is LocalBuilder { LocalIndex: var localIndex }))
			{
				if (!(operand is LocalVariableInfo { LocalIndex: var localIndex2 }))
				{
					if (!(operand is int result))
					{
						if (!(operand is byte result2))
						{
							if (!(operand is sbyte result3))
							{
								if (!(operand is short result4))
								{
									if (operand is ushort result5)
									{
										return result5;
									}
									return -1;
		...
		public static void CaughtFish_Postfix(Farmer __instance, string itemId, int size, bool from_fish_pond, int numberCaught)
		{
			try
			{
				...
				if (FishingRodPatches.TryGetPending(__instance, text, out var data) && !data.SuccessRecorded)
				{
					...
					int difficultyLevel = DifficultyManager.GetDifficultyLevel(text);
					int value = DifficultyManager.RecordSuccess(text, num);
					int difficultyLevel2 = DifficultyManager.GetDifficultyLevel(text);
					DifficultyManager.RecordHighDifficulty(text, data.AdjustedDifficulty);
					if (data.Multiplier > 15)
					{
						int fishSize = ((data.FishSize > 0) ? data.FishSize : (Math.Max(1, data.OriginalNum) * data.Multiplier));
						GiantFishManager.RecordGiantFish(text, data.Multiplier, fishSize);
					}
