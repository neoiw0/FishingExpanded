...
internal class BobberBarPatches
{
	...
	public static void Update_Postfix(BobberBar __instance, float ___distanceFromCatching, bool ___fadeOut, float ___bobberBarPos, ref float ___bobberPosition)
	{
		try
		{
			...
			else if (value.JumpWindupActive)
			{
	...
	public static void Draw_Postfix(BobberBar __instance, SpriteBatch b)
	{
		...
		try
		{
			...
			if (value.IsIdle)
			{
				...
				float num = (float)Math.Sin(totalMilliseconds / 1000.0 * Math.PI * 2.0) * 3f;
				DrawFishIconOverlay(b, __instance, 0f, new Vector2(num, 0f));
			}
			if (value.JumpWindupActive)
			{
			...
			if (value.HasChallengeBait && value.AdjustedDifficulty > 100f)
			{
				...
				if (challengeStars < 3)
				{
					...
					{
						b.Draw(Game1.mouseCursors_1_6, new Vector2((float)(num2 - 12), (float)(num3 + i * 40)) + __instance.everythingShake, (Rectangle?)value2, Color.White, 0f, Vector2.Zero, 2f, (SpriteEffects)0, 0.89f);
					}
				}
			}
	...
	private static void DrawFishIconOverlay(SpriteBatch b, BobberBar instance, float rotation, Vector2 offset = default(Vector2))
	{
		...
		Vector2 val = new Vector2((float)(((IClickableMenu)instance).xPositionOnScreen + 64 + 18), (float)(((IClickableMenu)instance).yPositionOnScreen + 12 + 24) + instance.bobberPosition) + instance.fishShake + instance.everythingShake + offset;
