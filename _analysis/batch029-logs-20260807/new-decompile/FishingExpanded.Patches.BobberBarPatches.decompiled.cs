...
internal class BobberBarPatches
{
	...
	public static void Constructor_Postfix(BobberBar __instance, string whichFish, ref float ___difficulty, ref int ___fishSize, ref int ___fishQuality, ref float ___distanceFromCatchPenaltyModifier, ref float ___bobberTargetPosition, bool ___bobberInBar)
	{
		try
		{
			...
			ModEntry.ModMonitor.Log($"[BobberBar] 钓鱼小游戏开始 | 实例: {((object)__instance).GetHashCode()} | 鱼ID: {text} | 难度等级: {difficultyLevel} | 原始difficulty: {num:F1} | 调整后: {___difficulty:F1} (×{difficultyMultiplier:F2}) | 数量倍数: {value} | fishSize: {___fishSize} | 品质: {___fishQuality} | 加速增幅档: {((difficultyLevel >= 89) ? "100%（等级≥89）" : "30%（等级≤88）")}", (LogLevel)2);
		}
	...
	public static float GetAccelerationBoost(BobberBar instance, float difficulty)
	{
		...
		InstanceData value;
		int num = (_instanceData.TryGetValue(instance, out value) ? value.DifficultyLevel : 89);
		float num2 = ((num >= 89) ? 1f : 0.3f);
