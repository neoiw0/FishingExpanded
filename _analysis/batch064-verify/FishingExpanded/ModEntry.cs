...
public class ModEntry : Mod
{
	...
	private void OnCommandDumpStars(string command, string[] args)
	{
		...
		try
		{
			...
			DumpRegion("cursors-achieve-star-area", Game1.mouseCursors, new Rectangle(128, 0, 48, 32), 8, text);
			...
			FishingLog.Log("✓ 贴图导出完成 → " + text + "（共 4 个 PNG）", (LogLevel)2);
		}
		catch (Exception value)
		{
	...
	private void OnCommandSelfTest(string command, string[] args)
	{
		...
		void Report(string name, bool flag8, string detail = "")
		{
			FishingLog.Log((flag8 ? "[PASS]" : "[FAIL]") + " " + name + ((detail.Length > 0) ? (" | " + detail) : ""), (LogLevel)(flag8 ? 2 : 4));
			if (flag8)
			{
				pass++;
			}
