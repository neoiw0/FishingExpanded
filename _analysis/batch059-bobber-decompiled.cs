...
internal class BobberBarPatches
{
	...
	private static void DrawTip(SpriteBatch b, FloatingTip tip)
	{
		...
		float maxWidth = (tip.RightAligned ? Math.Max(120f, tip.StartX - 16f) : Math.Max(120f, (float)((Rectangle)(ref Game1.viewport)).Width - tip.StartX - 16f));
		List<string> list = WrapTipText(dialogueFont, tip.Text, maxWidth, 1f);
		float num = (float)dialogueFont.LineSpacing * 1f;
		float num2 = (float)list.Count * num;
		float num3 = 0f;
		foreach (string item in list)
		{
			num3 = Math.Max(num3, dialogueFont.MeasureString(item).X * 1f);
		}
		float val3 = (tip.RightAligned ? (tip.StartX - num3) : (tip.Centered ? (tip.StartX - num3 / 2f) : tip.StartX));
		val3 = Math.Max(8f, Math.Min(val3, (float)((Rectangle)(ref Game1.viewport)).Width - 8f - num3));
		float num4 = (tip.Centered ? (tip.StartY + tip.YOffset) : (tip.StartY - num2 / 2f + tip.YOffset));
		Vector2 val4 = default(Vector2);
		for (int i = 0; i < list.Count; i++)
		{
			((Vector2)(ref val4))..ctor(val3, num4 + (float)i * num);
			b.DrawString(dialogueFont, list[i], val4 + new Vector2(2f, 2f), Color.Black * (tip.Alpha * 0.65f), 0f, Vector2.Zero, 1f, (SpriteEffects)0, 0f);
			for (int j = 0; j < 4; j++)
			{
				...
				b.DrawString(dialogueFont, list[i], val4 + val5, val2 * (tip.Alpha * 0.85f), 0f, Vector2.Zero, 1f, (SpriteEffects)0, 0f);
			}
			b.DrawString(dialogueFont, list[i], val4, Color.White * tip.Alpha, 0f, Vector2.Zero, 1f, (SpriteEffects)0, 0f);
		}
	}
		...
		try
		{
			...
			if (value.IdlePending && !value.IsIdle)
			{
				...
				if (___bobberPosition - 16f > num3 + 5f || ___bobberPosition + 12f < num2 - 5f)
				{
					...
					if (!value.IdleTipShown)
					{
						...
						AddTip(value.OtherTips, PickIdleText(), (float)___xPositionOnScreen + -44f, (float)___yPositionOnScreen + 12f + ___bobberBarPos + (float)___bobberBarHeight / 2f, centered: false, rightAligned: true);
					}
				}
			}
			...
			if (value.AdjustedDifficulty > 100f)
			{
				...
				if (num6 > 0)
				{
					...
					flag = num5 < DifficultyCalculator.GetCatchPenaltyModifier(value.DifficultyLevel, catchProgress2) - 0.0001f;
				}
			}
			...
			bool flag2 = num7 < value.NativeCatchPenaltyModifier - 0.0001f;
				...
				else
				{
					...
					if (value.JumpDetectionSeconds <= 0f)
					{
						...
						if (value.JumpPending)
						{
							...
							AddTip(value.ActionTips, text7, (float)___xPositionOnScreen + 208f, (float)___yPositionOnScreen + 36f + ___bobberPosition - 30f, centered: false, rightAligned: false, actionTip: true);
							if (value.PhraseMode == PhraseMode.Recording)
							{
								value.PhraseJumpIsUp = flag3;
	...
	public static void Draw_Postfix(BobberBar __instance, SpriteBatch b)
	{
		...
		try
		{
			...
			if (value.HasChallengeBait && value.AdjustedDifficulty > 100f)
			{
				...
				if (challengeStars < 3)
				{
					int num = (((float)((IClickableMenu)__instance).xPositionOnScreen > (float)((Rectangle)(ref Game1.viewport)).Width * 0.75f) ? (((IClickableMenu)__instance).xPositionOnScreen - 80) : (((IClickableMenu)__instance).xPositionOnScreen + 216));
					int num2 = (__instance.bobbers.Contains("(O)SonarBobber") ? (((IClickableMenu)__instance).yPositionOnScreen + 136) : (((IClickableMenu)__instance).yPositionOnScreen + 40));
					Rectangle value2 = default(Rectangle);
					((Rectangle)(ref value2))..ctor(217, 205, 19, 19);
					for (int i = challengeStars; i < 3; i++)
					{
						b.Draw(Game1.mouseCursors_1_6, new Vector2((float)(num - 12), (float)(num2 + i * 40)) + __instance.everythingShake, (Rectangle?)value2, Color.White, 0f, Vector2.Zero, 2f, (SpriteEffects)0, 0.89f);
					}
				}
			}
	...
	{
		float num = 0.88f - Math.Max(0f, data.JumpWindupSeconds);
		float num2 = ((!(num <= 0.77f)) ? (150f * (1f - (num - 0.77f) / 0.11f)) : (150f * (num / 0.77f)));
		return (jumpUp ? (-1f) : 1f) * num2;
	}
	...
	private static void HandlePhrasePrefix(BobberBar instance, InstanceData data, ref float position, ref float speed, ref float target, ref float floaterSinker, float dt)
	{
		...
		else
		{
			...
			if (!data.PlaybackWindupShown && data.PhraseWindupStartTime >= 0f && data.PhrasePlayTime >= data.PhraseWindupStartTime)
			{
				...
				{
					AddTip(data.ActionTips, data.PhraseJumpText, (float)((IClickableMenu)instance).xPositionOnScreen + 208f, (float)((IClickableMenu)instance).yPositionOnScreen + 36f + num - 30f, centered: false, rightAligned: false, actionTip: true);
				}
			}
