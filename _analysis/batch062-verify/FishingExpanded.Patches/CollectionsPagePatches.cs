...
internal class CollectionsPagePatches
{
	...
	public static void Draw_Postfix(SpriteBatch b, ClickableTextureComponent __instance, float layerDepth)
	{
		...
		try
		{
			...
			{
				b.Draw(Game1.mouseCursors, new Vector2((float)(((ClickableComponent)__instance).bounds.X + 3), (float)(((ClickableComponent)__instance).bounds.Y + 3)), (Rectangle?)new Rectangle(144, 16, 16, 16), Color.White, 0f, Vector2.Zero, 1.5f, (SpriteEffects)0, layerDepth + 0.01f);
				return;
			}
