...
internal class BobberBarPatches
{
	...
	private static void AddTip(List<FloatingTip> tips, string text, float x, float y, bool centered, bool rightAligned = false)
	{
		if (tips.Count >= 8)
		{
			tips.RemoveAt(0);
	...
	private static void DrawTip(SpriteBatch b, FloatingTip tip)
	{
		...
		{
			Vector2 val = Game1.smallFont.MeasureString(tip.Text);
			float num = (tip.RightAligned ? (tip.StartX - val.X) : (tip.Centered ? (tip.StartX - val.X / 2f) : tip.StartX));
			float num2 = (tip.Centered ? (tip.StartY + tip.YOffset) : (tip.StartY - val.Y / 2f + tip.YOffset));
			Vector2 val2 = default(Vector2);
			((Vector2)(ref val2))..ctor(num, num2);
			b.DrawString(Game1.smallFont, tip.Text, val2 + new Vector2(1f, 1f), Color.Black * (tip.Alpha * 0.7f));
			b.DrawString(Game1.smallFont, tip.Text, val2, Color.White * tip.Alpha);
		}
	}
	...
	public static void Update_Prefix(BobberBar __instance, ref float ___difficulty, ref float ___distanceFromCatchPenaltyModifier, ref float ___bobberPosition, ref float ___bobberTargetPosition, ref float ___bobberSpeed, float ___distanceFromCatching, bool ___bobberInBar, ref int ___fishSizeReductionTimer, ref int ___challengeBaitFishes, int ___xPositionOnScreen, int ___yPositionOnScreen, float ___bobberBarPos, int ___bobberBarHeight)
	{
		try
		{
			...
			float num3 = DifficultyCalculator.GetCatchPenaltyModifier(value.DifficultyLevel, ___distanceFromCatching);
			bool flag = false;
			int num4 = 0;
			if (!value.HasChallengeBait && value.AdjustedDifficulty > 100f)
			{
				...
				if (num4 > 0)
				{
					num3 = DifficultyCalculator.GetEscapeFailBonusModifier(value.DifficultyLevel, num4, ___distanceFromCatching);
					flag = num3 < DifficultyCalculator.GetCatchPenaltyModifier(value.DifficultyLevel, ___distanceFromCatching) - 0.0001f;
				}
			}
			...
			if (value.JumpPending)
			{
				...
				if (value.JumpPendingSeconds <= 0f)
				{
					...
					AddTip(value.ActionTips, text7, (float)___xPositionOnScreen + 208f, (float)___yPositionOnScreen + 36f + value.JumpPendingTarget - 30f, centered: false);
