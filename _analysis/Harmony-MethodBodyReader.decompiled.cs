...
internal class MethodBodyReader
{
	...
	internal void HandleNativeMethod()
	{
		...
		{
			AssemblyName assemblyName = new AssemblyName((methodInfo.DeclaringType?.FullName ?? "").Replace(".", "_") + "_" + methodInfo.Name);
			...
			MethodInfo operand = typeBuilder.CreateType().GetMethod(methodInfo.Name);
	...
	private void ParseExceptions()
	{
		foreach (ExceptionHandlingClause exception in exceptions)
		{
			...
			GetInstruction(tryOffset, isEndOfInstruction: false).blocks.Add(new ExceptionBlock(ExceptionBlockType.BeginExceptionBlock));
			GetInstruction(offset, isEndOfInstruction: true).blocks.Add(new ExceptionBlock(ExceptionBlockType.EndExceptionBlock));
			switch (exception.Flags)
			{
			case ExceptionHandlingClauseOptions.Filter:
				GetInstruction(exception.FilterOffset, isEndOfInstruction: false).blocks.Add(new ExceptionBlock(ExceptionBlockType.BeginExceptFilterBlock));
				break;
			case ExceptionHandlingClauseOptions.Finally:
				GetInstruction(handlerOffset, isEndOfInstruction: false).blocks.Add(new ExceptionBlock(ExceptionBlockType.BeginFinallyBlock));
				break;
			...
			case ExceptionHandlingClauseOptions.Fault:
				GetInstruction(handlerOffset, isEndOfInstruction: false).blocks.Add(new ExceptionBlock(ExceptionBlockType.BeginFaultBlock));
				break;
			}
		}
	}
	...
	internal List<CodeInstruction> FinalizeILCodes(Emitter emitter, List<MethodInfo> transpilers, List<Label> endLabels, out bool hasReturnCode)
	{
		...
		CodeTranspiler codeTranspiler = new CodeTranspiler(ilInstructions, argumentShift);
		...
		result.Do(delegate(CodeInstruction codeInstruction2)
		{
			...
			object obj2 = codeInstruction2.operand;
			...
			{
				CecilILGenerator obj3 = generator.GetProxiedShim<CecilILGenerator>() ?? throw new NotSupportedException();
				if (obj2 == null)
				{
					throw new Exception($"Wrong null argument: {codeInstruction2}");
				}
				if (!(obj2 is ICallSiteGenerator))
				{
					throw new Exception($"Wrong Emit argument type {obj2.GetType()} in {codeInstruction2}");
				}
				emitter.AddInstruction(opCode, obj2);
				emitter.LogIL(opCode, obj2);
				obj3.Emit(opCode, (ICallSiteGenerator)obj2);
				break;
			}
			default:
				if (obj2 == null)
	...
	private void ReadOperand(ILInstruction instruction)
	{
		...
		case OperandType.InlineSig:
		{
			...
			InlineSignature arg = (InlineSignature)(instruction.argument = (instruction.operand = InlineSignatureParser.ImportCallSite(module, array3)));
			...
			Debugger.Log(0, "TEST", $"Signature = {arg}\n");
			Debugger.Break();
			break;
		}
