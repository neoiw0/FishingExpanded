...
internal class BobberBarPatches
{
	...
	public static void Update_Prefix(BobberBar __instance, ref float ___difficulty, ref float ___distanceFromCatchPenaltyModifier, ref float ___bobberPosition, ref float ___bobberTargetPosition, ref float ___bobberSpeed, ref float ___floaterSinkerAcceleration, float ___distanceFromCatching, bool ___bobberInBar, ref int ___fishSizeReductionTimer, ref int ___challengeBaitFishes, int ___xPositionOnScreen, int ___yPositionOnScreen, float ___bobberBarPos, int ___bobberBarHeight)
	{
		try
		{
			...
			if ((!(value.EffectiveDifficulty >= 150f) && !value.JumpPending) || value.ResultStarted || value.IsIdle)
			{
				return;
			}
	...
	[HarmonyPatch("update")]
	[HarmonyPostfix]
	public static void Update_Postfix(BobberBar __instance, float ___distanceFromCatching, bool ___fadeOut, float ___bobberBarPos)
	{
		try
		{
			if (!_instanceData.TryGetValue(__instance, out var value))
	...
	public static void Draw_Postfix(BobberBar __instance, SpriteBatch b)
	{
		...
		try
		{
			...
			if (value.IsIdle)
			{
				...
				float rotation = (float)Math.Sin(totalMilliseconds / 450.0) * 0.09f;
				DrawFishIconOverlay(b, __instance, rotation);
			}
			if (value.JumpWindupActive)
			{
				bool jumpUp = value.JumpPendingTarget <= 133f;
				float jumpWindupRotation = GetJumpWindupRotation(value, jumpUp);
				DrawFishIconOverlay(b, __instance, MathHelper.ToRadians(jumpWindupRotation));
			}
			if (value.HasChallengeBait && value.AdjustedDifficulty > 100f)
			{
				int challengeStars = GetChallengeStars(value.DifficultyLevel, value.BattleElapsedSeconds);
	...
	private static void DrawFishIconOverlay(SpriteBatch b, BobberBar instance, float rotation)
	{
		...
		((Rectangle)(ref value))..ctor(614 + (instance.bossFish ? 20 : 0), 1840, 20, 20);
		Vector2 val = new Vector2((float)(((IClickableMenu)instance).xPositionOnScreen + 64 + 18), (float)(((IClickableMenu)instance).yPositionOnScreen + 12 + 24) + instance.bobberPosition) + instance.fishShake + instance.everythingShake;
		b.Draw(Game1.mouseCursors, val, (Rectangle?)value, Color.White, rotation, new Vector2(10f, 10f), 2f, (SpriteEffects)0, 0.885f);
	}

	private static float GetJumpWindupRotation(InstanceData data, bool jumpUp)
	{
		float num = 0.22f - Math.Max(0f, data.JumpWindupSeconds);
		float num2 = ((!(num <= 0.14f)) ? (40f * (1f - (num - 0.14f) / 0.08f)) : (40f * (num / 0.14f)));
