...
internal class FishingRodPatches
{
	...
	public static float GetFishVisualScale(FishingRod rod)
	{
		try
		{
			...
			return DifficultyCalculator.GetVisualScale(data.Multiplier);
		}
	...
	public static void CreateFish_Postfix(FishingRod __instance, ref Item __result)
	{
		try
		{
			...
			if (flag && (value.WildBaitBonus || value.ChallengeBonusActive || value.ChallengeStarMultiplier < 1f || value.Multiplier > 1))
			{
