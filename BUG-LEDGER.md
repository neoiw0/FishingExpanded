...
5. 五只鱼王不参与难度、数量、品质、奖励、技能加成、称号、视觉和 NPC 规则；统一的20%/1%全局蓄力槽保护仍适用于鱼王。

### 当前维护门禁

- v0.5.2 的历史记录包含构建和部署事实，但九项玩家验收仍未完成；每个现象必须独立标记通过、失败或未执行。
- 当前优先验证：视觉放大、单鱼动画/背包数量/单次结算、脱杆收益与区间封顶、0级全局蓄力保护、鱼王豁免与全局保护、原生 `ItemGrabMenu` 溢出。
