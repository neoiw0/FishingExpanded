...
14. 隐藏等级加成整体删除；Mod 鱼皇冠只显示、不计手感进度；原版 5 条传奇鱼（159/160/163/682/775）钓到一次直接给皇冠，扩展传奇 898–902 按普通鱼规则（2026-08-09，BATCH-034）。
15. 手感进度 α = 玩家已收集可计数皇冠数 ÷ 61（56 普通 + 5 原版传奇，Mod 鱼不计）；α=0 原生手感、α=1 终点手感（按下/松开瞬间定速 30px/帧、无加速度/条内阻尼/惯性、撞边完全钳制），0→61 皇冠全程线性分配；绿条外钳制机制取消（2026-08-09，BATCH-034）。
16. 帧率解耦覆盖整个钓鱼小游戏（绿条、鱼、平滑、概率），60fps 行为与原生一致；加速度增幅等级 <98 → 现增幅 10%、≥98 → 100%（2026-08-09，BATCH-034；2026-08-09 晚用户确认该档位公式正确，BATCH-036 曾临时统一 10% 已还原）。
17. 跳鱼瞬移时显示贴鱼文案：上跳（目标 0~133）“鱼跃”类 15 条随机、下跳（目标 399~532）“甩尾！”类 15 条随机，停留 1 秒后 0.5 秒淡出（2026-08-09，BATCH-034）。
...
19. 图鉴皇冠加成显示（BATCH-036，2026-08-09 用户确认）：有皇冠的鱼在描述中追加“钓鱼控制力+1”（i18n collections.crownControl；中英双语）；非鱼王同时保留“挑战等级”行，鱼王豁免挑战等级但皇冠行照常显示；纯展示，不新增状态与存档字段。
...
22. 提示双通道（BATCH-038）：鱼动作提示（鱼跃/甩尾）与鱼其他提示（助战/力竭等）各为独立浮动提示通道；起点固定、显示 2 秒、2 秒内上移 30px 并线性淡出、可同时多条、每通道上限 8 丢最旧；其他提示锚定钓鱼条左侧（x+44）；纯显示不入 FIFO 队列；助战文案由“1 秒停留+0.5 秒淡出”改为浮动提示通道（BATCH-035 文案池沿用）。
...
- BATCH-038（2026-08-10 用户多轮确认的钓鱼改版，见 `Governance\BATCH-038-Exhaust-Tips-ChallengeBait.md`）：力竭机制（难度≥100 非鱼王，节点 1/3/5/7/9/12/15 分钟 1/3/10/20/35/50/100% 线性，15 分钟难度降到 80，奖励按开局快照不衰减；每节点 10 套诙谐文案进“鱼其他提示”）；提示双通道（动作/其他各 2 秒上移 30px 淡出、起点固定、上限 8）；脱杆惩罚取消（难度等级>0 尺寸+品质，等级照旧 10/5/2/1）；尺寸修复（狗鱼 100 级根因=原生 800ms 缩水，结算边界统一乘倍率并同步收藏记录）；万能鱼饵 +10（等级>0 且原生两条）；挑战鱼饵改版（难度>100，5 分钟 ×1.5 数量，超时只取消数量加成，难度不衰减、无助战、原生 3 次脱杆禁用、节点提示+10 条追加文案）；流动金色皇冠（≥95 级挑战鱼饵成功，新增 ChallengeCrowns 存档字段）；手感 α 优化（定速 -30% + 方向系数 ×1.2/×0.8 线性）；100 级太一 + 满皇冠人鱼合一史诗文案。实现+构建完成（0 警告 0 错误，DLL `9D0F1DF8...`，harness 26/26 + Transpiler TYPE SIM OK），未部署，验收项每现象独立标记。
...
- **任务**：用户一次提交 5 项确认修改（详见 `Governance\BATCH-033-CrownBonus-NPCcm-BarEntry-FloatText.md`）：① NPC 惊叹与手持尺寸一致显示厘米（fishSize×2.54）；② 王冠加成 0.5→0.2（旧存档加载自动重算生效）；③ 难度等级 ≥80 且鱼在绿条外时，鱼自身随机运动不得进入绿条（玩家移条照常可抓、150+ 跳鱼瞬移不受影响、鱼王/非鱼类天然豁免）；④ 低难度鱼不显示挑战宣言（第一因已证实：BATCH-032 测试命令 fish_addstars 无难度门槛写星标，产生低难度鱼持有星标的非法状态，宣言忠实消费；用户新规则=宣言须同时满足难度等级≥1 且有星标）；⑤ 浮木→陷阱浮标/软木塞浮标文案。
...
- **任务**：用户多轮确认的玩法改版（详见 `Governance\BATCH-034-Feel-System-Rework.md`）：① 隐藏等级加成整体删除（Mod 鱼无任何加成只显示皇冠）；② 皇冠/手感进度 α=可计数皇冠÷61（56 普通+5 原版传奇；传奇一次钓获直接给皇冠）；③ 取消绿条外钳制（BATCH-033 机制整体退休）；④ 手感系统（终点：按下/松开瞬间定速 30px/帧、无加速度/阻尼/惯性、撞边完全钳制，0→61 线性分配）；⑤ 帧率解耦覆盖整个小游戏；⑥ 加速度增幅 <98→10%、≥98→100%；⑦ 跳鱼瞬移文案（鱼跃/甩尾各 15 条）。
