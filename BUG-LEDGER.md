...
63. **蟹笼收获纳入难度等级（BATCH-065，2026-08-15 用户指令，见 `Governance\Active\BATCH-065-CrabPot-Difficulty.md`）**：蟹笼里面拿到的物品也加难度等级，一律按水藻类（非鱼）规则处理——①每次收获 = 一次成功，等级 +1（上限 8，8 级给图鉴星星 + "无手感增强"，封顶"你已是帝王"）；②收获应用数量倍数（BATCH-062 公式=难度等级×1，8 级=×8，与原生书《Crabbing》25%×2 叠加=×16）；③左下角提示只在等级实际提升时显示（平时不刷屏）；④蟹笼鱼（Data/Fish 带 `trap` 标签、Category=-4：372/715-723 等）经 `DifficultyManager.IsCrabPotFish` 显式归入非鱼判定（IsNonFishItem/GetMaxDifficultyLevel/HUD 封顶文案统一单一所有者）；⑤新 Patch `CrabPotPatches`（`CrabPot.checkForAction` 收获唯一权威点：Prefix 应用倍数+登记待结算，Postfix 入包成功才 `RecordSuccess`+1；背包满收获取消不结算）；⑥经验保持原生固定 5 点、不实施每日限额（每笼每天 1 次，天然远低于 333，理由见活动卡）；⑦顺带同步 GAME-DESIGN 数量倍数公式到 BATCH-062（等级×1）与经验解耦措辞。fish_selftest 增 5 条蟹笼只读断言。Release Rebuild ✅（0 警告 0 错误，DLL SHA256 `833A0812A16D73670AD9BBE9A6162A17FF656B7CE542500A56AAC5209072E2D0`，反编译核验全符号编译进 DLL，证据 `_analysis\batch065-verify`）；**状态修正（2026-08-16 治理核对）：实际已部署**——该哈希与当前安装 `D:\GGGGG\K1515\Mods\FishingExpanded\FishingExpanded.dll` 一致（即 BATCH-060v2 统一构建 `833A0812...` 2026-08-15 21:29 部署时已含蟹笼符号，安装目录反编译含 CrabPotPatches/IsCrabPotFish），活动卡"未部署"记录过期；待真实验收（fish_selftest 蟹笼 5 条 + 真实蟹笼收获：等级+1、8 级星标+帝王提示、×8 数量）。
64. **节日钓鱼开关（BATCH-066，2026-08-16 用户指令，见 `Governance\Active\BATCH-066-Festival-Fishing-Toggle.md`）**：允许玩家设置钓鱼节日是否使用模组；默认关闭=节日完全原生（当前行为=节日全用模组，默认值反转是用户明确要求）；覆盖三个钓鱼节日——①鱿鱼节（SquidFest，冬 12-13 沙滩，被动节日，分数=`SquidFestScore += numberCaught` 原生）；②鳟鱼大赛（TroutDerby，夏 20-21 森林，被动节日，tag 概率原生 33%×numCaught，不走 CreateFish）；③冰雪节（Festival of Ice，冬 8 森林冰湖钓鱼比赛，**事件型节日**——`Game1.isFestival()`=true 走 `Event.caughtFish` 而非 `Farmer.caughtFish`，`fishCaught=false` 鱼不进背包，`festivalScore++` 每次成功 +1 分，经验原生跳过；用户确认纳入）。实现：`ModConfig.EnableFestivalFishingMods`（默认 false）+ GMCM 开关 + 新服务 `FestivalFishingService`（IsSquidFest/IsTroutDerby/IsIceFestival/IsVanillaFestivalMode）+ 6 处节日门（BobberBar 构造、pullFishFromWater Prefix、CreateFish Postfix、CaughtFish Prefix/Postfix、gainExperience Prefix）+ 分数补差（鱿鱼节 `SquidFestScore += numberCaught×(倍数−1)`；冰雪节新 `Event.caughtFish` Postfix `festivalScore += (倍数−1)` 并消费式移除 pending——顺带修复潜伏缺陷：冰雪节记录 PendingFishData 但无人消费的残留）；`fish_selftest` 增 3 条只读断言；i18n×2 `config.festivalFishing.*`。夜市潜艇钓鱼（冬 15-17）为 FishingGame 小游戏，无 FishingRod 链，模组不介入无需开关（取证 `_analysis\festival-tmp\`）。R0/R1/R2 完成；Release Rebuild ✅（0 警告 0 错误，DLL SHA256 `6F681270C170EC6230C7536A5088EFFB0E1DC981970D8B2E9B1F487CBC1B2283`，反编译核验全符号编译进 DLL，证据 `_analysis\batch066-verify`）；**未部署**（待用户授权）；待真实验收（真实鱿鱼节/冰雪节/鳟鱼大赛，见活动卡集中测试顺序）。**2026-08-16 07:45 部署状态更新：已随 BATCH-067 统一候选部署**（同一 DLL `CDCCA63F...`，备份 `DeploymentBackups\FishingExpanded-20260816-074434-pre-BATCH067-066`，i18n×2 同步含 `config.festivalFishing.*` 键，config.json 未触碰——`EnableFestivalFishingMods` 缺失由 ModConfig 默认 false 兜底）；待真实验收（同活动卡集中测试顺序）。
65. **难度等级挂钩固定钓鱼等级 + 成功保底 +1 + 挑战掉星折扣口径回归（BATCH-067，2026-08-16 用户逐条确认，见 `Governance\Active\BATCH-067-FishingLevel-ScaledGain-Floor1-StarDiscount.md`）**：①**难度等级增长与固定钓鱼等级挂钩**（系数 = max(等级,1)×0.1：1 级 ×0.1、10 级 ×1.0=现有速度；0 级钳 0.1；只读 `Farmer.fishingLevel` 字段——不含食物/饮料 buff 与助战临时等级，原生证据 Farmer.decompiled.cs:592-593/1612）——`请求增益 = max(1, round((脱杆基数 + round(调整后难度/50)) × 系数))`，保留 BATCH-029 雪球并整体乘系数（用户确认）；蟹笼固定 +1 不吃系数（用户确认）；②**成功保底 +1 级**（0 级新档/多脱杆低系数时 round 归 0 由 max(1,…) 兜底；已封顶 100/8 仍为 +0 正确行为）；③**挑战掉星折扣口径回归设计**——原生取证（BobberBar.decompiled.cs:240-242/354-357 challengeBaitFishes=3、FishingRod.decompiled.cs:2403-2418 Stack=numberOfFishCaught）证实掉星路径 nativeStack 恒=3，`round(3×0.4)=1` **不存在归零 bug**（先前 0 条鱼假设已被原生证据排除并更正）；实际缺陷=折扣先作用于原生 3 条再乘倍数（FishingRodPatches.cs:481-482），与 GAME-DESIGN §7.5"每掉 1 颗**最终鱼获** −20%"口径不符——修复为对乘完等级倍数后的最终数量打折并兜底 ≥1（倍数 10+0 星：10→12 条；低倍数无差异）；失败−1/89+ 每次 +6/挑战不掉级/非鱼上限 8/鱼王豁免均不动（用户确认）。实现：`DifficultyCalculator` 新增 3 个纯函数（`GetFishingLevelGainFactor`/`GetRequestedLevelGain`/`ApplyChallengeStarMultiplier`）+ `CaughtFish_Postfix`/`CreateFish_Postfix` 接入 + 日志追加系数与请求增长字段；`fish_selftest` 增 11 条只读断言；无新增状态/存档字段。R0/R1/R2 完成；**统一构建完成（DLL SHA256 `CDCCA63FA36E7C940DC8732EA71F719F3D113328E2BB0DFBE8E871EA74385993`，0 警告 0 错误，与 BATCH-066 合并统一构建，反编译核验通过，证据 `_analysis\batch067-verify`）**；**2026-08-16 07:45 已部署**（用户授权"仅部署，先不测试"；备份 `DeploymentBackups\FishingExpanded-20260816-074434-pre-BATCH067-066`=833A0812...，源/目标哈希一致，i18n×2+manifest 同步，config.json 未触碰）；待真实验收（用户明确先不测试；测试顺序见活动卡：fish_selftest 11 条 + 真实钓鱼 1 级 vs 10 级完美钓 +1/+10、0 级保底 +1、挑战 0 星数量公式）。
66. **经验基数钳制 [原生难度, 120] + 经验倍数曲线线性 ×5→×20（BATCH-068，2026-08-16 用户排查要求 + 确认方案 A、上限 120、线性曲线，见 `Governance\Active\BATCH-068-Experience-Base-Compensation.md`）**：用户要求排查"我们的系统导致玩家获得经验比原本少"——**已证实两条路径**：①负数难度等级（-1~-10，难度 ×0.5~0.95，`BobberBarPatches.cs:510` 构造乘法）使原生经验公式（`FishingRod.decompiled.cs:1188-1203`：`max(1, (品质+1)×3 + 难度/3)`，宝箱 +120%/完美 +140%/Boss ×5）难度输入变小，而经验倍数负数=1 不补偿（-10 级金枪鱼 26→14，少 46%）；②力竭衰减（`BobberBarPatches.cs:690-697` 非挑战鱼饵把 `___difficulty` 改写为有效难度降到 80）对原生难度 >80 的扩展传奇（898=95/899=85/900=110/902=100，`Fish-data-extracted.txt:69-73`）经验 < 原生（Legend II 39→29）；GAME-DESIGN §7.1 "奖励不下降"快照未覆盖经验基数。**修复①+②（方案 A，只补"变少"）**：`GainExperience_Prefix` 在 `传入难度 < 原生难度 || 传入难度 > 120` 时按 `GetNativeExperience(品质, GetExperienceDifficulty(传入, 原生), 宝箱, 完美, Boss)` 重算（复刻原生公式，钳制 [原生,120]）再走经验倍数；`PullFishFromWater_Prefix` 签名扩展记录原生难度/传入难度/品质/宝箱/完美/Boss（PendingFishData 6 个新字段，消费即弃，无存档变化）；`BobberBarPatches.GetOriginalDifficulty` 只读方法。**③经验倍数膨胀**（讨论确认后用户指令"10难度能力还是乘以5，之后平滑处理，100难度等级乘以20"）：`GetExperienceMultiplier` 改为分段——1~10 级保持 max(1, round(level×0.5, AwayFromZero))（10 级 = ×5 现状），10~100 级线性 `round(5 + (level−10)×15/90)`（100 级 = ×20，原 ×50），≥100 封顶 20；100 级总经验从 58450→2150（钳制前）/2150→860（钳制+新倍数）；膨胀从"基数×倍数双爆炸"收敛为"线性 ×20"。`fish_selftest` 增 17 条只读断言（经验重算 5 + 钳制 5 + 倍数曲线 7）。R0/R1/R2 完成；**已统一构建（DLL SHA256 `F5DA3733CBA06B8079C98F40F671BA2422B935CD4A45D2B6F3AE3B2E297F8895`，0 警告 0 错误，反编译核验通过，证据 `_analysis\batch068-verify`；前次构建 `12DD9D97...` 因倍数修改作废未部署）；2026-08-16 11:52 已部署**（轮询任务 pwsh-41 在检测到游戏退出前被终止（exit 1073807364），随后确认游戏已退出直接部署；备份 `DeploymentBackups\FishingExpanded-20260816-115228-pre-BATCH068v2`=CDCCA63F...，源/目标 DLL 哈希一致，i18n×2+manifest 一致，config.json 未触碰（F7F2E2FB... 游戏写入版含 `EnableFestivalFishingMods=false`））；待真实验收（fish_selftest 17 条 + 负数金枪鱼=26 + Legend II 力竭=39 + 50/100 级日志 ×12/×20）。

67. **VanillaTips API 顶层接口修复（BATCH-069，2026-08-16 用户日志反证，见 `Governance\Active\BATCH-069-VanillaTips-TopLevelApi.md`）**：BATCH-060 第 9 项部署版被反证——已安装 VanillaTips 时 SMAPI 报 `Tried to map a mod-provided API to non-public interface 'FishingExpanded.Services.VanillaTipsIntegration+IVanillaTipsApi'`，随后误判“VanillaTips 未安装”跳过 7 条提示注入；GCE 通过同一 API 注入成功证明 VanillaTips API 本身可用。**根因已证实**：`IVanillaTipsApi` 被声明为 `VanillaTipsIntegration` 的嵌套接口，SMAPI `ModRegistry.GetApi<T>` 要求顶级 public 接口（`Type.IsPublic`），嵌套 public 只是 `IsNestedPublic` 会被拒绝。**修复**：把 `IVanillaTipsApi` 移到 `FishingExpanded.Services` 命名空间顶层，`TryRegister` 调用不变；R0/R1/R2 完成（静态契约，无运行逻辑增减）。**已构建并部署（DLL SHA256 `E4C964EA79FE902DBF4115BEE3545DB1D1A55486472978D95C13E71A3E8551C4`，0 警告 0 错误，反射核验 `IsPublic=True IsNested=False`；2026-08-16 14:58 部署，备份 `DeploymentBackups\FishingExpanded-20260816-145849-pre-BATCH069`=F5DA3733...，源/目标哈希一致，config.json 未触碰）**；待真实验收（SMAPI 日志出现“已向 VanillaTips 注入 7 条提示”且无 `non-public interface` 错误）。

68. **提示按 1/3 屏宽换行（BATCH-070，2026-08-16 用户指令，见 `Governance\Active\BATCH-070-Prompt-Wrap-ThirdScreen.md`）**：各种提示在英文下可能太长；用户确认范围=仅本 Mod 自绘提示、口径=UI 视口宽度/3。实现：①BobberBar 浮动提示把固定 `TipMaxWidthPixels=420f` 改为 `Game1.uiViewport.Width / 3f`（保留两侧可用空间约束）；②HUDNotifier/ModEntry 的 HUD 消息新增 `WrappingHUDMessage` 子类，按 `Game1.uiViewport.Width / 3` 用 `Game1.parseText` 换行并动态增高 HUD 气泡背景，避免英文长文案超屏（含 ModEntry GMCM 重置完成消息）。纯显示修改，不改变玩法/存档/多人状态；BATCH-069 与本批统一部署、由用户一并验收。**首版已构建并部署（DLL `B2E49624...`，2026-08-16 20:39）；用户反馈左下角 HUD 换行后气泡高度不够，修复为背景高度=文本高度+上下边距并拉伸左侧气泡底图（DLL `88DB2F7B32AEBB85D8B8C6DEC4DE6A9DB702453B5E0B8A21D5728ABBA3AAD552`，0 警告 0 错误，2026-08-16 21:14 部署；备份 `DeploymentBackups\FishingExpanded-20260816-211423-pre-BATCH070-fix`=B2E496...，源/目标哈希一致，config.json 未触碰）**；待真实验收（英文长浮动提示与 HUD 通知在 1/3 屏宽内换行且气泡完整包裹文字）。**2026-08-17 用户反证：左下角 HUD 文本≥3 行时仍略超出气泡；源码已改为用 `Game1.smallFont.MeasureString(wrapped).Y` 计算真实文本高度（原 `lines.Length * LineSpacing` 会低估中文字形高度），Release 构建 `05554B169E0D77E649028B9A54931F06439EC2B85C7777CB70905E9F3ED448F7` 0 警告 0 错误；2026-08-17 22:15 已部署（备份 `DeploymentBackups\FishingExpanded-20260817-221555-pre-BATCH070-heightfix2`=2367A9AE...；源/目标哈希一致；config.json 未触碰）**。
69. **文案标点归一化（BATCH-071，2026-08-16 用户指令，见 `Governance\Active\BATCH-071-Punctuation-Normalization.md`）**：所有面向玩家的文案中全角括号 `（）` 改为半角 `()`；英文文案中的宽破折号 `—`/`\u2014` 改为英文连字符 `-`（按上下文保留空格）。范围= `Source/i18n/zh.json` + `Source/i18n/default.json` + 源码中的用户可见字符串（ModEntry/BobberBar/FishingRod/GiantFishManager/HUDNotifier/VanillaTips）。纯文案/显示字符串修改，不改变玩法、存档、config.json 或运行时行为；免除诊断与自动化运行验收，仅需构建/部署/哈希核验。**已部署（DLL SHA256 `2367A9AE01AFC54E64A554523F8A6BF5BC756727BDAC2E6EFFFC12E61A4F4BA6`，0 警告 0 错误；备份 `DeploymentBackups\FishingExpanded-20260817-134030-pre-BATCH071`；源/目标 DLL+i18n 哈希一致；config.json 未触碰）**；待真实验收（中英文案标点显示；与 BATCH-069/070 一并验收）。
70. **强制下一次鱼种测试命令（BATCH-072，2026-08-18 用户指令，见 `Governance\Active\BATCH-072-ForceNextFish.md`）**：新增 `fish_next [玩家序号] <鱼ID>`——为指定玩家（可为主屏/副屏序号）设置下一次钓鱼小游戏固定鱼种，BobberBar 构造边界消费一次，会话态不写存档；支持 `fish_next 151` / `fish_next 2 151`。实现：BobberBar 构造函数 Prefix 在原生创建前替换 `whichFish` 为未限定 ID（如 `151`），并清空原鱼临时 `setFlagOnCatch`；`fish_selftest` 新增标志生命周期自测；文档同步 TESTING-GUIDE/WIKI/TESTING。Release Rebuild ✅（0 警告 0 错误，DLL SHA256 `1E65AB958AC0873A48DC86DCB967A532DAEE4D52CE96C3B5967D094ACCEDF245`）；**2026-08-18 已部署**（备份 `DeploymentBackups\FishingExpanded-20260818-084900-pre-BATCH072`=05554B16...，源/目标 DLL 哈希一致，i18n 源/目标一致，manifest/config.json 未触碰）；待真实验收（`fish_next 151` / `fish_next 2 151`）。
71. **英文单位·动物叫声·60秒布丁·金冠助战·自定义称号（BATCH-073，2026-08-18 用户逐条确认，见 `Governance\Active\BATCH-073-ENUnits-LocalizedSounds-Perseverance60-CrownAssist-CustomTitle.md`）**：①英文模式所有 cm 改 in.（NPC 冒泡/对话），英文用原生 fishSize 英寸数字，其他语言保持厘米；②动物叫声英文化且保持多种随机，只改头顶冒泡；③战斗 ≥60 秒海泡布丁概率 60%（40% 无奖励、不叠加 30 秒档）；④助战总概率 = 基础 + 每条流动金冠 +0.1% + 每条增大流动金冠（100 级 1.2 倍）再 +0.05%，触发后按权重选鱼（普通 1 / 流动 1.001 / 增大 1.0015）；⑤`ModConfig.CustomFishingTitle`（默认空、不注册 GMCM）空值回退 i18n 称号、非空替代所有显示点。Release Rebuild ✅（0 警告 0 错误，DLL SHA256 `34929524F2EA5458FD7762C29157FECB7F9BE4074FC91DE3BAE26560E62981F1`）；**2026-08-18 15:15 已部署**（备份 `DeploymentBackups\FishingExpanded-20260818-151519-pre-BATCH073`；源/目标 DLL+i18n 哈希一致，config.json 未触碰）；待真实验收。

72. **巨型鱼重置 + 联机同步 + 困难模式皇冠（BATCH-074，2026-08-18 用户确认，见 `Governance\Active\BATCH-074-GiantFish-Reset-MultiplayerSync-CrownHardMode.md`）**：①CAT-A 进 FarmHouse/换日重置超大鱼展示+赞美（修复进房后同种鱼不再冒泡、换日展示残留；`FishDisplayData.ResetSessionEffects` 统一清展示+赞美，旧 `ResetDailyTriggers`/`ClearActiveGiantFish` 退休）；②CAT-B 困难模式（`EnableRandomFishBehavior=true`，随机无背板）100 级流动皇冠图鉴绘制 1.2→1.3 倍（仅视觉，助战权重不变）；③CAT-C 联机同步（分屏+远程）：新增 SMAPI 多人消息 `GiantFishRecord`/`GiantFishClear`/`NPCFishBubble`，超大鱼视觉与 NPC 冒泡同步到其他玩家屏幕，私有 HUD 不广播；已取证 `Game1.drawPlayerHeldObject(f)`→`drawWhenHeld(...,f)` 覆盖所有玩家手持绘制。实施顺序=AB→静态验证→C→静态验证→统一部署；CAT-A/B/C 代码侧结束，Release 构建 0 警告 0 错误，DLL SHA-256 `9F2E9BDB5D7D078B034A79841FA6052024F41904E485E1B417FC35BE1839088B`；**2026-08-18 18:49 已部署**（备份 `DeploymentBackups\FishingExpanded-20260818-184957-pre-BATCH074`；源/目标 DLL+i18n 哈希一致；manifest 未同步；config.json 未触碰）；**2026-08-18 用户确认后续统一使用 `neoiw`（`neoiw.FishingExpanded`），本轮已部署故不重新部署，下次部署时同步安装目录 manifest 的 UniqueID**；待真实验收。
