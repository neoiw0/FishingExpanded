...
- 历史 v0.5.2 记录包含旧构建/部署事实；当前源码版本已统一为 `0.5.10`，本轮构建/部署仍不替代真实存档测试。
- BATCH-024 的 R1 修正已完成并部署 `0.5.10`：多人玩家数据隔离、原生 `CreateFish` 数量边界、鱼王 ID 归一化、成功后星标和巨型鱼展示隔离均已进入待实测状态；在线联机主机与每位农场客都必须安装本 Mod。
...
- 多人主机/农场客独立难度、星标和技能加成；正常背包与原生 `ItemGrabMenu` 都只在最终物品创建时转换数量。
